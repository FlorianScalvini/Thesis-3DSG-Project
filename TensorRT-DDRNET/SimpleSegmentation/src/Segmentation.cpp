...
Segmentation::Segmentation(const std::string &pathNetwork) {
...
void Segmentation::loadNetwork(const std::string &pathNetwork) {
...
void Segmentation::doInference(cv::Mat img) {
    int i = 0;
    cv::Mat resizeImg;
    if( _pData == nullptr)
        return;
...
void Segmentation::destroy() {

}

// Size of the input image
cv::Size Segmentation::getInputSize()
{
    return cv::Size{_widthInput, _heightInput};
}

// Size of the segmented image
cv::Size Segmentation::getOutputSize()
{
