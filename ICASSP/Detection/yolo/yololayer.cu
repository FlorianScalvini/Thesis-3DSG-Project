...
namespace nvinfer1
{
    YoloLayerPlugin::YoloLayerPlugin(int classCount, int netWidth, int netHeight, int maxOut, const std::vector<Yolo::YoloKernel>& vYoloKernel)
    {
        ...
        mYoloNetWidth = netWidth;
        mYoloNetHeight = netHeight;
    ...
    YoloLayerPlugin::YoloLayerPlugin(const void* data, size_t length)
    {
        ...
        read(d, mThreadCount);
        read(d, mKernelCount);
        read(d, mYoloNetWidth);
        read(d, mYoloNetHeight);
    ...
    void YoloLayerPlugin::serialize(void* buffer) const TRT_NOEXCEPT
    {
        ...
        write(d, mYoloNetWidth);
        write(d, mYoloNetHeight);
    ...
    {
        return sizeof(mClassCount) + sizeof(mThreadCount) + sizeof(mKernelCount) + sizeof(Yolo::YoloKernel) * mYoloKernel.size() + sizeof(mYoloNetWidth) + sizeof(mYoloNetHeight) + sizeof(mMaxOutObject);
    }
    ...
    {
        YoloLayerPlugin* p = new YoloLayerPlugin(mClassCount, mYoloNetWidth, mYoloNetHeight, mMaxOutObject, mYoloKernel);
    ...
    void YoloLayerPlugin::forwardGpu(const float* const* inputs, float *output, cudaStream_t stream, int batchSize)
    {
        ...
        for (unsigned int i = 0; i < mYoloKernel.size(); ++i) {
            ...
            CalDetection<<<(numElem + mThreadCount - 1) / mThreadCount, mThreadCount, 0, stream>>>
                    (inputs[i], output, numElem, mYoloNetWidth, mYoloNetHeight, mMaxOutObject, yolo.width, yolo.height, (float*)mAnchor[i], mClassCount, outputElem);
        }
    }
