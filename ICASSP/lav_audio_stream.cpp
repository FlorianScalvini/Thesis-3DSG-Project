...
void lavAudioStream::init()
{
    ...
    lavLog::LOG("bufferSize", bufferSize);

    //lavLog::LOG("Set audio parameters", bufferSize);
	if ((err = snd_pcm_hw_params (playback_handle, hw_params)) < 0) {
		fprintf (stderr, "cannot set parameters (%s)\n",
			 snd_strerror (err));
		exit (1);
	}
    //lavLog::LOG("Audio parameters successfully set");
    ...
    //lavLog::LOG("exit lav_audio_stream_init\n");
...
void* lavAudioStream::play_audio_stream(void* arg) {
    ...
    while (1) {//period_size) {
        ...
        if (result<0) {
            lavLog::LOG("ERROR in lavAudioStream::play_audio_stream", result);
            if (result == -77) {
                lavLog::LOG("PCM is not in the right state (SND_PCM_STATE_PREPARED or SND_PCM_STATE_RUNNING) ");
            }
            else if (result == -32) {
                lavLog::LOG("an underrun occurred");
            }
            else if (result == -86) {
                lavLog::LOG("a suspend event occurred (stream is suspended and waiting for an application recovery)");
            }
        }
        cptLoop++;
	/*
        if (cptLoop>1000) {
	    lavLog::LOG("Bug volontaire dans lav_audio_stream.cpp methode playAudioStream");
            int test=5;
            lavLog::LOG("cpt", test/0);
        }
	*/
    }
