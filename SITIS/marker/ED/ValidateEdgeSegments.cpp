...
///
static short *ComputeLSD(unsigned char *srcImg, int width, int height, double *H){
...
///
static void TestSegment(EdgeMap *map, short *gradImg, int i, int index1, int index2, int np, double *H, double div){
...
/// We take pixels at Nyquist distance, i.e., 2 (as suggested by DMM)
///
static void TestSegment2(EdgeMap *map, short *gradImg, int i, int index1, int index2, int np, double *H, double div){
  unsigned char *edgeImg = map->edgeImg;
...
#define MIN_PATH_LEN 10
  ...
  for (int k=index1; k<=index2; k++){
    ...
    // Count the number of pixels at Nyquist distance from each other
    if (k == index1){count1++; continue;}
    int dx = abs(map->segments[i].pixels[k].c - map->segments[i].pixels[k-1].c);
    int dy = abs(map->segments[i].pixels[k].r - map->segments[i].pixels[k-1].r);    
    if (dx == 1 && dy == 1) count2++; // diagonal
    else                    count1++; // one pixel left/right/up/down
  } //end-for
...
//    TestSegment2(map, gradImg, i, 0, map->segments[i].noPixels-1, np, H, divForTestSegment);
  } //end-for
...
///
void ValidateEdgeSegments2(EdgeMap *map, unsigned char *srcImg, double divForTestSegment){
  int width = map->width;
  int height = map->height;
  ...
  for (int i=1; i<height-1; i++){
    for (int j=1; j<width-1; j++){
      ...
      G = abs(srcImg[(i-1)*width+j-1] + srcImg[(i-1)*width+j] + srcImg[(i-1)*width+j+1] - 
              srcImg[(i+1)*width+j-1] - srcImg[(i+1)*width+j] - srcImg[(i+1)*width+j+1]);
      if (G > g) g = G;

      // 45 degrees
      G = abs(srcImg[i*width+j+1] + srcImg[(i+1)*width+j+1] + srcImg[(i+1)*width+j] - 
              srcImg[i*width+j-1] - srcImg[(i-1)*width+j-1] - srcImg[(i-1)*width+j]);
      if (G > g) g = G;

      // 90 degrees
      G = abs(srcImg[(i-1)*width+j+1] + srcImg[i*width+j+1] + srcImg[(i+1)*width+j+1] - 
              srcImg[(i-1)*width+j-1] - srcImg[i*width+j-1] - srcImg[(i+1)*width+j-1]);
      if (G > g) g = G;

      // 135 degrees
      G = abs(srcImg[(i-1)*width+j] + srcImg[(i-1)*width+j+1] + srcImg[i*width+j+1] - 
              srcImg[(i+1)*width+j] - srcImg[(i+1)*width+j-1] - srcImg[i*width+j-1]);
      if (G > g) g = G;
#else
      // Sobel
      int g=0, G;
      ...
      // 0 degrees
      G = abs(srcImg[(i-1)*width+j-1] +  2*srcImg[(i-1)*width+j] + srcImg[(i-1)*width+j+1] -
              srcImg[(i+1)*width+j-1] -  2*srcImg[(i+1)*width+j] - srcImg[(i+1)*width+j+1]);
      if (G > g) g = G;

      // 45 degrees
      G = abs(srcImg[(i-1)*width+j] +  2*srcImg[(i-1)*width+j-1] + srcImg[i*width+j-1] -
              srcImg[(i+1)*width+j] -  2*srcImg[(i+1)*width+j+1] - srcImg[i*width+j+1]);
      if (G > g) g = G;

      // 90 degrees
      G = abs(srcImg[(i-1)*width+j+1] +  2*srcImg[i*width+j+1] + srcImg[(i+1)*width+j+1] -
              srcImg[(i-1)*width+j-1] -  2*srcImg[i*width+j-1] - srcImg[(i+1)*width+j-1]);
      if (G > g) g = G;
...
///
int ValidateEdgeSegmentsMultipleDiv(EdgeMap *map, unsigned char *srcImg, unsigned char *maps[], int noMaps){
  int width = map->width;
  int height = map->height;
  ...
  int index = 0;
  for (double validationDiv=1; validationDiv <= 8.5; validationDiv += 0.5, index++){
    ...
    // Validate segments
    for (int i=0; i<map->noSegments; i++){
      TestSegment(map, gradImg, i, 0, map->segments[i].noPixels-1, np, H, validationDiv);
    } //end-for
...
///
void ValidateEdgeSegments(EdgeMap *map, unsigned char *ch1Img, unsigned char *ch2Img, unsigned char *ch3Img, double divForTestSegment){
  int width = map->width;
  int height = map->height;
  ...
  for (int i=1; i<height-1; i++){
    for (int j=1; j<width-1; j++){
      ...
      int com1 = ch1Img[(i+1)*width+j+1] - ch1Img[(i-1)*width+j-1];
      int com2 = ch1Img[(i-1)*width+j+1] - ch1Img[(i+1)*width+j-1];

      int gxCh1 = abs(com1 + com2 + (ch1Img[i*width+j+1] - ch1Img[i*width+j-1]));
      int gyCh1 = abs(com1 - com2 + (ch1Img[(i+1)*width+j] - ch1Img[(i-1)*width+j]));
      ...
      int gxCh2 = abs(com1 + com2 + (ch2Img[i*width+j+1] - ch2Img[i*width+j-1]));
      int gyCh2 = abs(com1 - com2 + (ch2Img[(i+1)*width+j] - ch2Img[(i-1)*width+j]));
      ...
      int gxCh3 = abs(com1 + com2 + (ch3Img[i*width+j+1] - ch3Img[i*width+j-1]));
      int gyCh3 = abs(com1 - com2 + (ch3Img[(i+1)*width+j] - ch3Img[(i-1)*width+j]));
...
///--------------------------------------------------------------------------------------------------------------------
/// Validate the edge segments using the Helmholtz principle (for color images) channel1, channel2 and channel3 images with LSD
///
void ValidateEdgeSegments2(EdgeMap *map, unsigned char *ch1Img, unsigned char *ch2Img, unsigned char *ch3Img, double divForTestSegment){
  int width = map->width;
  int height = map->height;
  ...
  for (int i=1; i<height-1; i++){
    ...
    // Use DiZenZo's gradient vector gradient computation
      ...
      // Magnitude
//      int grad = (int)(sqrt(gxx*cos(theta)*cos(theta) + 2*gxy*sin(theta)*cos(theta) + gyy*sin(theta)*sin(theta)) + 0.5);  // Where did I get this from?
      int grad = (int)(sqrt( ((gxx+gyy) + (gxx - gyy)*cos(2*theta) + 2*gxy*sin(2.0*theta))/2.0) + 0.5);  // Image Processing Book??    
#endif
...
/// Validate the edge segments using the Helmholtz principle (for color images) channel1, channel2 and channel3 images
///
int ValidateEdgeSegmentsMultipleDiv(EdgeMap *map, unsigned char *ch1Img, unsigned char *ch2Img, unsigned char *ch3Img, unsigned char *maps[], int noMaps, int op){
  int width = map->width;
  int height = map->height;
  ...
  if (op == 1){
    for (int i=1; i<height-1; i++){
      for (int j=1; j<width-1; j++){
        ...
        // Take average
//        int grad = (ch1Grad + ch2Grad + ch3Grad + 2)/3;  // works better
...
//  size -= grads[0];
  ...
  int index = 0;
  for (double validationDiv=1; validationDiv <= 8.5; validationDiv += 0.5, index++){  // Orig loop -- good results
    unsigned char *contourImg = maps[index];
...
///
void ValidateEdgeSegmentsWithGradientMap(EdgeMap *map, short *gradImg, double divForTestSegment){
  int width = map->width;
  int height = map->height;
