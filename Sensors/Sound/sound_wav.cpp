...
SoundWav::SoundWav(const char *filepath) : filename(filepath)
{
    sizeData = -1;
    sizeHeader = -1;
...
unsigned int SoundWav::getSizeData() const{
    return this->sizeData;
}

unsigned int SoundWav::getSizeHeader() const{
    return this->sizeHeader;
}

unsigned int SoundWav::getNbChannel() const {
    return this->header.num_channels;
}

unsigned int SoundWav::getNbbytes() const
{
...
int SoundWav::writeStandardWavFile(const char* filename, char* format, char* data, unsigned int nbByte) {
    ...
    unsigned int copied_nb_frames = nbByte/nbBytePerSample;
...
void SoundWav::readData(short* data, unsigned int nbItem) {
    ...
    else if(header.bits_per_sample == 8)
    {
        ...
        {
            data[i] = static_cast<short>(dataChar[i]);
        }
