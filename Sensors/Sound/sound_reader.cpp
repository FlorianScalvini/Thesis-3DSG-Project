...
int SoundReader::init(const char* file, int sizechunks) {
    if(bufferSound != nullptr)
    {
        delete [] bufferSound;
    }
    bufferSound = nullptr;
    ...
    {
        printf("Error, the file %s is not a monophonic sound", file);
        return EXIT_FAILURE;
    }
    numberChunks = ceil((sizeData) / (double) sizechunks);
