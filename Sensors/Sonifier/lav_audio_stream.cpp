...
void lavAudioStream::init()
{
    ...
    SonifierLogger::logger((char *)"bufferSize", bufferSize);
...
void* lavAudioStream::play_audio_stream(void* arg) {
    ...
    while (1) {//period_size) {
        ...
        if (result<0) {
            //SonifierLogger::logger((char *)"ERROR in lavAudioStream::play_audio_stream", result);
            if (result == -77) {
                SonifierLogger::logger((char *)"PCM is not in the right state (SND_PCM_STATE_PREPARED or SND_PCM_STATE_RUNNING) ");
            }
            else if (result == -32) {
                //SonifierLogger::logger((char *)"an underrun occurred");
            }
            else if (result == -86) {
                SonifierLogger::logger((char *)"a suspend event occurred (stream is suspended and waiting for an application recovery)");
            }
        }
        cptLoop++;
	/*
