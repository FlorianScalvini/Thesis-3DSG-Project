...
namespace nvinfer1
{
    class API ArgMaxPlugin : public IPluginV2IOExt
    {
    public:
        ArgMaxPlugin(int netChannels, int netHeight, int netWidth, int axis);
        ArgMaxPlugin(const void* data, size_t length);
        ~ArgMaxPlugin();
        ...
        void forwardGpu(const float* inputs, float *output, cudaStream_t stream, int batchSize = 1);
    ...
    class API ArgMaxPluginCreator : public IPluginCreator
    {
    public:
        ArgMaxPluginCreator();
